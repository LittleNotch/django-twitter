...
class HBaseModel:
    ...
    def deserialize_row_key(cls, row_key):
        ...
        # val1:val2 => val1:val2: every time find(':') can get a val
        row_key = row_key + ':'
    ...
    def serialize_field(cls, field, value):
        ...
        if isinstance(field, IntegerField):
            # dictionary sort, 1 10 2
            # fix int digits to 16 bits, add '0'
            value = str(value)
            while len(value) < 16:
                value = '0' + value
    ...
    def save(self):
        ...
        # if row_data is None, no column key values need to store
        # raise exception,
        if len(row_data) == 0:
